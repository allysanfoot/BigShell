...
builtin_exit(struct command *cmd, struct builtin_redir const *redir_list)
{
  ...
  size_t wordCount = cmd->word_count;
  // Exit if [n] is omitted
  if (wordCount < 2)
  {
    exit(params.status);
  }
  // Obtain [n] from command string
  long ret = strtol(cmd->words[1], NULL, 10);
  ...
  err:
    dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "Invalid argument for exit!\n");
    bigshell_exit();
    return -1;
}
