...
get_io_flags(enum io_operator io_op)
{
  ...
  case OP_GREAT:                         /* > */
    ...
    break;
  case OP_DGREAT:                          /* >> */
    flags = O_APPEND | O_WRONLY | O_CREAT; /* TODO */
    break;
  case OP_LESSGREAT: /* <> */
    flags = O_RDWR;  /* TODO */
    break;
  case OP_CLOBBER:                        /* >| */
    flags = O_TRUNC | O_CREAT | O_WRONLY; /* TODO */
    break;
  }
  return flags;
}
...
int run_command_list(struct command_list *cl)
{
  ...
  for (size_t i = 0; i < cl->command_count; ++i)
  {
    ...
    assert(child_pid > 0);
    if (stdout_override >= 0)
      close(stdout_override);
    if (stdin_override >= 0)
      close(stdin_override);
     ...
     * 2) pipeline_pgid != 0
     */
    if (setpgid(child_pid, pipeline_pgid) < 0)
      goto err;
    if (pipeline_pgid == 0)
    {
